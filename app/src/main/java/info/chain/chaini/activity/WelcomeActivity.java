...
public class WelcomeActivity extends Activity {
    ...
    private final Runnable mWelcomeRunnable = new Runnable() {
        @Override
        public void run() {
            String user = Utils.getSpUtils().getString("firstUser");
            if (user == null) {
                goLogin();
            } else {
                UserBean userBean = MyApplication.getDaoInstant().getUserBeanDao().queryBuilder().where(UserBeanDao.Properties.User_name.eq(user)).build().unique();
                if (userBean == null) {
                    goLogin();
                } else {
                    goHome();
    ...
    private void delayedWelcome(int delayMillis) {
        mHideHandler.removeCallbacks(mWelcomeRunnable);
        mHideHandler.postDelayed(mWelcomeRunnable, delayMillis);
    }
